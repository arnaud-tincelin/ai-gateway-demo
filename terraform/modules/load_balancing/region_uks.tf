module "foundry_uks" {
  source            = "../foundry"
  unique_name       = "lb-uks-${var.unique_name}"
  apim_principal_id = var.apim_principal_id
  resource_group_id = var.resource_group_id
  location          = "uk south"
}

resource "azurerm_cognitive_deployment" "uks" {
  name                 = local.deployment_name
  cognitive_account_id = module.foundry_uks.aifoundry_id
  sku {
    name     = "GlobalStandard"
    capacity = 1
  }
...
resource "azapi_resource" "apim_backend_uks" {
  ...
  body = {
    properties = {
      url         = "${module.foundry_uks.aifoundry_endpoint}openai"
      protocol    = "http"
      description = "Inference backend UKS"
