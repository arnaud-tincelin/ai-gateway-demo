...
module "foundry_frc" {
  source            = "../foundry"
  unique_name       = "lb-frc-${var.unique_name}"
  apim_principal_id = var.apim_principal_id
  resource_group_id = var.resource_group_id
  location          = "france central"
}

resource "azurerm_cognitive_deployment" "frc" {
  name                 = local.deployment_name
  cognitive_account_id = module.foundry_frc.aifoundry_id
  sku {
    name     = "GlobalStandard"
    capacity = 1
  }
...
resource "azapi_resource" "apim_backend_frc" {
  ...
  body = {
    properties = {
      url         = "${module.foundry_frc.aifoundry_endpoint}openai"
      protocol    = "http"
      description = "Inference backend FRC"
